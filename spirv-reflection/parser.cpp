...
UbuElement parseElement(const spirv_cross::CompilerGLSL &compiler, const spirv_cross::SPIRType &type, uint32_t index, std::vector<UboStruct> &uboStructs);

void parseStruct(const spirv_cross::CompilerGLSL &compiler, const std::string &name, const spirv_cross::SPIRType &type, std::vector<UboStruct> &uboStructs) {
  UboStruct uboStruct = {};
  uboStruct.name = name;
  const auto member_count = type.member_types.size();
  if(member_count == 0) {
    parseElement(compiler, type, 0, uboStructs);
  }
  else {
    for(uint32_t i = 0; i < member_count; i++) {
      auto element = parseElement(compiler, type, i, uboStructs);
      uboStruct.elements.push_back(element);
    }
  }
  uboStructs.push_back(uboStruct);
}
...
UbuElement parseElement(const spirv_cross::CompilerGLSL &compiler, const spirv_cross::SPIRType &type, uint32_t index, std::vector<UboStruct> &uboStructs) {
  ...
  if(type.basetype == spirv_cross::SPIRType::BaseType::Struct && member_type.member_types.size() > 0) {
    ...
    parseStruct(compiler, structName, member_type, uboStructs);
...
Ubos parseUbos(const spirv_cross::CompilerGLSL &compiler, const spirv_cross::ShaderResources &resources) {
  ...
  for(const auto &resource : resources.uniform_buffers) {
    ...
    ubo.sizeInBytes = uint32_t(compiler.get_declared_struct_size(type));
    ubo.name = resource.name;

    const auto member_count = type.member_types.size();
    for(uint32_t i = 0; i < member_count; i++) {

      const auto element = parseElement(compiler, type, i, ubo.uboStructs);
...
bool parsePushConstant(const spirv_cross::CompilerGLSL &compiler, const spirv_cross::ShaderResources &resources) {
  return resources.push_constant_buffers.size() > 0;
}

VertexInputs parseVertexInput(const spirv_cross::CompilerGLSL &compiler, const spirv_cross::ShaderResources &resources) {
  ...
  for(auto &resource : resources.stage_inputs) {
    ...
    if(type.basetype == spirv_cross::SPIRType::BaseType::Float) {
      switch(type.vecsize) {
      case 1: format = InputFormats::FORMAT_R32_SFLOAT; break;
      case 2: format = InputFormats::FORMAT_R32G32_SFLOAT; break;
      case 3: format = InputFormats::FORMAT_R32G32B32_SFLOAT; break;
      case 4: format = InputFormats::FORMAT_R32G32B32A32_SFLOAT; break;
      default:
        assert(false && "Input format not supported"); break;
      }
      vertexInput.size = sizeof(float) * type.vecsize;
    }
    else if(type.basetype == spirv_cross::SPIRType::BaseType::Int) {
      switch(type.vecsize) {
      case 1: format = InputFormats::FORMAT_R32_SINT; break;
      case 2: format = InputFormats::FORMAT_R32G32_SINT; break;
      case 3: format = InputFormats::FORMAT_R32G32B32_SINT; break;
      case 4: format = InputFormats::FORMAT_R32G32B32A32_SINT; break;
      default:
        assert(false && "Input format not supported"); break;
      }
  ...
  std::sort(std::begin(vertexInputs), std::end(vertexInputs), [](const auto &a, const auto &b) {
...
DescriptorSets parseDescriptorSets(const spirv_cross::CompilerGLSL &compiler, const spirv_cross::ShaderResources &resources) {
  ...
  for(auto &resource : resources.sampled_images) {
    ...
    descriptorSet.descriptorSetType = DescriptorSetTypes::COMBINED_IMAGE_SAMPLER;
...
Shader parseSpirv(const std::string &name, const std::string &path, const std::string &type) {
...
Shader parseShader(const std::string &name, const std::string &path) {
  ...
  for(uint32_t i = 0; i < vertexShader.descriptorSets.size(); i++) {
    for(uint32_t j = 0; j < fragSize; j++) {
      ...
      if(d1.name == d2.name) {
        assert(std::tie(d1.name, d1.set, d1.location) == std::tie(d2.name, d2.set, d2.location));
        goto CONTINUE;
      }
    }
  ...
  std::sort(std::begin(fragmentShader.descriptorSets), std::end(fragmentShader.descriptorSets), [](const auto &a, const auto &b) {
  ...
  vertexShader.hasPushContant = vertexShader.hasPushContant || fragmentShader.hasPushContant;
